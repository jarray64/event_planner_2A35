...
//****************************************************
QSqlQueryModel * Societe::triesociete(QString test)
{
    QSqlQueryModel * model=new QSqlQueryModel();
    if(test == "par défaut"){
        model->setQuery("SELECT * from marketing");
    ...
    {
        model->setQuery("SELECT * from marketing order by mail asc ");
    }
    else if(test =="facebook")
    {
        model->setQuery("SELECT * from marketing order by fcb desc ");
    }
    else if(test =="nom")
    {
        model->setQuery("SELECT * from marketing order by nom asc ");
    }
    else if(test =="contact")
    {
        model->setQuery("SELECT * from marketing order by contact asc ");
    }
    return model;
}


QSqlQueryModel * Societe::recherchersocite(QString chaine)
{
    QSqlQueryModel * model= new QSqlQueryModel();
    model->setQuery("select * from MARKETING where nom LIKE '"+chaine+"%'");
    return model;
}
