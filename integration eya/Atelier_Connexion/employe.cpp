...
bool Employe::ajouter()
 {
     ...
     QSqlQuery query;
          query.prepare("INSERT INTO G_PERSONNELS (nom, keyperso, prenom,pass,age) "
...
QSqlQueryModel * Employe ::afficher(int a,int b)
{
    ...
    if(a==0){
        if(b==1)
    model->setQuery("select * FROM G_PERSONNELS ORDER BY keyperso ");
        else
                model->setQuery("select * FROM G_PERSONNELS ORDER BY keyperso DESC");
    }else if(a==1){
                if(b==1)
    model->setQuery("select * FROM G_PERSONNELS ORDER BY nom");
                else
                        model->setQuery("select * FROM G_PERSONNELS ORDER BY nom DESC");
    }else if(a==-1){
    model->setQuery("select * FROM G_PERSONNELS ");

    }

    else {
                   if(b==1)
     model->setQuery("select * FROM G_PERSONNELS ORDER BY age ");   else

     model->setQuery("select * FROM G_PERSONNELS ORDER BY age DESC");
           }
...
checkQuery.prepare("select * from G_PERSONNELS where keyperso=:keyperso ");
...
checkQuery.bindValue(":keyperso",keyperso);
 ...
 {
     QSqlQuery query;
     query.prepare("Delete from G_PERSONNELS where keyperso=:keyperso");
     query.bindValue(":keyperso", keyperso);
  return   query.exec();
}
 ...
 {
     QSqlQuery query;
     query.prepare("update  G_PERSONNELS set  nom=:nom, age=:age,prenom=:prenom ,pass=:pass where keyperso=:keyperso " );
     query.bindValue(":keyperso", keyperso);
     query.bindValue(":nom", nom);
     query.bindValue(":prenom", prenom);
     query.bindValue(":pass", pass);
...
{
     QSqlQueryModel * model= new QSqlQueryModel();
 model->setQuery("SELECT * FROM G_PERSONNELS WHERE KEYPERSO LIKE '"+recherche+"%' OR age LIKE '"+recherche+"%' OR NOM LIKE '"+recherche+"%'");
 ...
 void  Employe::genererPDF(){
    ...
    int i = 4000;
         ...
         QSqlQuery query;
         query.prepare("select * from G_PERSONNELS");
         query.exec();
         while (query.next())
         {
 ...
 {
 QSqlQueryModel* model=new QSqlQueryModel();
 model->setQuery("select * from G_PERSONNELS order by "+colone+" "+ordre+"");
 return model;
 }
